...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const menuItems: { id: AppTab; label: string; icon: React.ReactNode; badge?: number }[] = [
    ...
    },
    {
      id: 'templates',
      label: 'Templates',
      icon: <LayoutTemplate className="w-5 h-5 shrink-0 text-amber-400" />,
    },
    {
      id: 'extension',
      label: 'Extensão',
        ...
        <div className="p-3 border-t border-stone-800 bg-stone-950/50">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2.5 overflow-hidden">
              <div className="w-9 h-9 rounded-xl bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 font-bold flex items-center justify-center text-xs shrink-0">
            ...
            {(isExpanded || mobileOpen) && (
              <button
                onClick={onLogout}
                title="Sair da Conta"
                className="p-2 text-stone-400 hover:text-red-400 hover:bg-red-950/40 rounded-lg transition-all shrink-0"
              >
